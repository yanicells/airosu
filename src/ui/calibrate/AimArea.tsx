  ...
  return (
    <div ref={root} className="aim-editor">
      <div
        className="aim-area"
        style={{
          left: `${(area.cx - area.halfW) * 100}%`,
          top: `${(area.cy - area.halfH) * 100}%`,
          width: `${area.halfW * 200}%`,
          height: `${area.halfH * 200}%`,
        }}
      >
        <button
          type="button"
          className="aim-area__move"
          aria-label="Move aim area"
          aria-describedby="aim-help"
          {...interaction('move')}
        >
          <span>Aim area</span>
        </button>
        {handles.map((handle) => (
          <button
            key={handle}
            type="button"
            className={`aim-area__handle aim-area__handle--${handle}`}

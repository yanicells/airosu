...
const video = {
  paused: false, readyState: 2, currentTime: 0,
  requestVideoFrameCallback: vi.fn((cb) => { frame = cb; return 1; }),
...
it('runs on camera frames and drops frames while inference is busy', async () => {
  ...
  tracker.detect.mockReturnValueOnce(new Promise((r) => { resolve = r; }));
...
it('closes a tracker that finishes starting after stop', async () => {
  ...
  vi.mocked(createHandTracker).mockReturnValueOnce(new Promise((r) => { resolve = r; }));
  const source = createHandCursorSource();
...
it('zero smoothing emits the current position without filter delay', async () => {
  ...
  await source.start(video as unknown as HTMLVideoElement);
  tracker.detect.mockResolvedValue({ landmarks: Array.from({ length: 21 }, () => ({ x: 0.5, y: 0.5 })) });
  frame(10);
  await Promise.resolve();
  tracker.detect.mockResolvedValue({ landmarks: Array.from({ length: 21 }, () => ({ x: 0.75, y: 0.75 })) });
  frame(30);
  await Promise.resolve();

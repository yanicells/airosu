...
export function cameraBoxToDisplay(box: CalibrationBox, sensitivity: number, mirror: boolean): CalibrationBox {
  return { cx: mirror ? 1 - box.cx : box.cx, cy: box.cy, halfW: box.halfW / sensitivity, halfH: box.halfH / sensitivity };
}

export function displayBoxToCamera(box: CalibrationBox, sensitivity: number, mirror: boolean): CalibrationBox {
  return { cx: mirror ? 1 - box.cx : box.cx, cy: box.cy, halfW: box.halfW * sensitivity, halfH: box.halfH * sensitivity };
}

export function adjustAimArea(box: CalibrationBox, handle: AimAreaHandle, dx: number, dy: number): CalibrationBox {
  if (handle === 'move') {
    return { ...box, cx: clamp(box.cx + dx, box.halfW, 1 - box.halfW), cy: clamp(box.cy + dy, box.halfH, 1 - box.halfH) };
  }
  let left = box.cx - box.halfW, right = box.cx + box.halfW;
  let top = box.cy - box.halfH, bottom = box.cy + box.halfH;
  ...
  return { cx: (left + right) / 2, cy: (top + bottom) / 2, halfW: (right - left) / 2, halfH: (bottom - top) / 2 };
}

/** Only count samples near the chosen target, not while moving toward it. */
export function isAtCalibrationCorner(point: Vec2, box: CalibrationBox, corner: 'top-left' | 'bottom-right', sensitivity: number, mirror: boolean): boolean {
  const area = cameraBoxToDisplay(box, sensitivity, mirror);
  const sign = corner === 'top-left' ? -1 : 1;
  const x = mirror ? 1 - point.x : point.x;
  return Math.abs(x - (area.cx + sign * area.halfW)) <= Math.min(0.06, area.halfW * 0.5)

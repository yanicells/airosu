...
page). Also ship a small licensed starter library, persist uploaded maps
locally, and let the player aim with either their palm or index fingertip.
...
│ IndexedDB: your .osz maps │              │ components: aggregate,       │
│ game engine (unchanged)   │              │   migrations                 │
└───────────────────────────┘              │                              │
                                           └───────┬──────────────────────┘
...
auth/submit), `src/ui/leaderboard/`, `src/ui/profile/`, `src/ui/nav/`, and
...
starter-manifest validation, weighted-total math, submission
...
`convex-test`. UI (navbar, profile, leaderboard) verified
...
- Automatic beatmap downloading from osu! or unofficial mirrors. Reconsider
  only if osu! exposes a supported third-party download API.
